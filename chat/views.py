...
class ContactView(LoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            contect, created = Contact.objects.get_or_create(
                owner=request.user)
            if user != request.user:
                contect.member.add(user)
                try:
                    ContactRequest.objects.create(sender=request.user, receiver=user, created_at=timezone.now(), updated_at=timezone.now())
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            messages = Message.objects.filter(
                room_member__room=room).order_by('created_at')
            return render(request, 'messages.html', {'messages': messages, 'room': room})
...
def get_room_or_send_invitation(request):
    if request.method == 'POST':
        ...
        try:
            ...
            contact_request = ContactRequest.objects.filter(Q(sender=request.user, receiver=end_user)|Q(sender=end_user, receiver=request.user))
            if len(contact_request):
                contact_request = contact_request[0]
            else:
                contact_request = ContactRequest.objects.create(sender=request.user, receiver=end_user)
            if contact_request.accepted == True:
                room = Room.objects.filter(Q(roommember__member=end_user)).filter(
                    Q(roommember__member=request.user)).filter(room_type='P')
                if len(room):
                    room = room[0]
                    return JsonResponse({'message':'User Found', 'room_id':room.room_number, 'status':200 })
                else:
...
def list_invitation(request):
    if request.method == 'GET':
        contact_request_list = ContactRequest.objects.filter(receiver=request.user, accepted=False)
        if len(contact_request_list):
            return render(request, 'notification.html', {'contact_request_list':contact_request_list})
        else:
...
def accept_invitation(request):
    if request.method == 'POST':
        ...
        try:
            ...
            # creating room
            room = Room.objects.filter(Q(roommember__member=invitation.sender)).filter(
                    Q(roommember__member=invitation.receiver)).filter(room_type='P')
            if not len(room):
                room = Room.objects.create(room_type="P")
                room_member = RoomMember.objects.create(member=invitation.sender, is_admin=True, room=room)
                room_member = RoomMember.objects.create(member=request.user, is_admin=True, room=room)

                contact, created = Contact.objects.get_or_create(owner=invitation.receiver)
...
def create_group(request):
    if request.method == 'POST':
        ...
        if group_name and group_description and group_image:
            room = Room.objects.create(name=group_name, description=group_description, image=group_image, room_type='G')
            room_member = RoomMember.objects.create(member=request.user, room=room, is_admin=True, is_online=True)
...
def manage_group(request):
    if request.method == 'GET':
        ...
        try:
            ...
            return render(request, 'manage_group.html', {'room':room, 'room_member_list':room_member_list, 'current_member':room_member})
        except Exception as ex:
            print(ex)
...
def group_member_add_list(request):
    if request.method == 'GET':
        ...
        try:
            ...
            if room_member.is_admin == True:
                contact, created = Contact.objects.get_or_create(owner=request.user)
                contact_list = contact.member.all().exclude(id__in=list(RoomMember.objects.filter(room=room).values_list('member', flat=True)))
                print(contact_list)
                html_response = ''
                if len(contact_list):
                    for member in contact_list:
                        html_response += '<option value="{0}">{1}</option>'.format(member.id, member.get_full_name())
            ...
            if len(member_list):
                for member in member_list:
                    try:
                        ...
                        room_member, created = RoomMember.objects.get_or_create(member=user, room=room)
                        if created:
...
def remove_group_member(request):
    if request.method == 'POST':
        ...
        try:
            ...
            current_room_member = RoomMember.objects.get(member=request.user, room=room_member.room)
            room_number = room_member.room.room_number
            try:
                other_room_members = RoomMember.objects.filter(room=room_member.room).exclude(member=room_member.member).order_by('created_at')
